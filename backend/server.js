// Enable legacy OpenSSL provider for Node.js 17+
const crypto = require('crypto');
const { webcrypto } = require('crypto');

// Log Node.js and OpenSSL version
console.log(`\n📦 Node.js version: ${process.version}`);
console.log(`🔐 OpenSSL version: ${crypto.getOpenSSLVersion ? crypto.getOpenSSLVersion() : 'unknown'}\n`);

// Check if legacy provider is available
try {
    // Try to use MD5 (legacy algorithm)
    const hash = crypto.createHash('md5');
    hash.update('test');
    hash.digest('hex');
    console.log('✅ Legacy crypto algorithms are supported');
} catch (err) {
    console.error('❌ ERROR: Legacy crypto not supported. Run with: node --openssl-legacy-provider server.js');
    process.exit(1);
}
...
const aesRoutes = require('./src/routes/aesRoutes');
const desRoutes = require('./src/routes/desRoutes');
